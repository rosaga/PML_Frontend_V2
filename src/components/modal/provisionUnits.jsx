...
import { autoProvisionUnits, getBalance } from "@/app/api/actions/provisionUnits/provisionUnits";

const ProvisionUnitsModal = ({ closeModal, orgUnitId, org_unit_name }) => {
  let org_id = null;
  ...
  const handleRequest = async () => {
    try {
      ...
      if (allRequests.length === 0) {
        setErrorMessage("Please add at least one request");
        setIsLoading(false);
        return;
      }
      ...
      if (results.every(result => result)) {
        setSuccessMessage(`Your Data Units have been auto-provisioned successfully${org_unit_name ? ` for ${org_unit_name}` : ''}`);
        setErrorMessage("");
        //clear
        setRequests([]);
        setBundleAmount("");
        ...
        setErrorMessage("Failed to provision units. Please try again.");
      }
    } catch (error) {
        ...
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
          <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
              {org_unit_name ? `Provision Units for ${org_unit_name}` : 'Provision Units'}
            </h3>
                ...
                <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                  ...
                  <div className="flex space-x-2">
                    ...
                    <button
                      ...
                      disabled={isLoading}
                      className={`w-full text-white ${isLoading ? 'bg-orange-300' : 'bg-orange-400 hover:bg-orange-500'} focus:ring-4 focus:outline-none focus:ring-orange-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-orange-500 dark:hover:bg-orange-600 dark:focus:ring-orange-800`}
                    >
...
export default ProvisionUnitsModal;