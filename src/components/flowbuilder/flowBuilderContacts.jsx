...
import ContactSessionsModal from "../modal/ContactSessionsModal"; 
import { getToken } from "@/utils/auth";
import { GetContacts, GetFlowbuilderContacts } from "../../app/api/actions/contact/contact"
...
const UploadRecipients = () => {
  ...
  const [isSessionsModalOpen, setIsSessionsModalOpen] = useState(false); // Add this state
  const [selectedContact, setSelectedContact] = useState(null); // Add this state
  ...
  const getContacts = async () => {
    try {
      const res = await GetFlowbuilderContacts(org_id, paginationModel.page+1, paginationModel.pageSize, searchParams);
      if (res.errors) {
  ...
  // Add function to close sessions modal
  ...
  const deleteContact = async (id) => {
    ...
    const deleteUrl = `https://flowbot-1048592730476.europe-west4.run.app/api/v2/contacts/${org_id}/contact/${id}`
    try {
  ...
  return (
    <>
      ...
      {isSessionsModalOpen && selectedContact && (
        <ContactSessionsModal 
          closeModal={closeSessionsModal} 
